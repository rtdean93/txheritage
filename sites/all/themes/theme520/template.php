<?php
function theme520_menu_local_task($link, $active = FALSE) {
  return '<li '. ($active ? 'class="active" ' : '') .'><span><span>'. $link ."</span></span></li>\n";
}

function theme520_l($text, $path, $options = array()) {
  // Merge in defaults.
  $options += array(
      'attributes' => array(),
      'html' => TRUE,
...
*/
function theme520_links($links, $attributes = array('class' => 'links')) {
 $output = '';
  if (count($links) > 0) {
...
// In the line above, you could take out the drupal_attributes var and specify your class
       ...
       $output .= theme520_l($spanned_title, $link['href'], $link['attributes'], $link['query'], $link['fragment']);
      } else if ($link['title']) {
